/**
 * Orangerail action `deleteShop` (Prisma delete on model `Shop`).
 *
 * This file is yours — re-scans never modify it; `orangerail sync` reports drift.
 * Write operation: staged for human approval; on approval it runs `prisma.shop.delete(...)` against your database.
 * DESTRUCTIVE: permanently deletes a `Shop` row on approval — an approver should confirm the identifier before authorizing.
 */
import { z } from 'zod';
...
export const deleteShop = registry.defineAction({
  ...
  execute: async ({ input }) => {
    try {
      ...
      return await prisma.shop.delete({ where: { "id": input["id"] } });
    } catch (error) {
