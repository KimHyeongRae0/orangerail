/**
 * Orangerail action `deleteOrder` (Prisma delete on model `Order`).
 *
 * This file is yours — re-scans never modify it; `orangerail sync` reports drift.
 * Write operation: staged for human approval; on approval it runs `prisma.order.delete(...)` against your database.
 * DESTRUCTIVE: permanently deletes a `Order` row on approval — an approver should confirm the identifier before authorizing.
 */
import { z } from 'zod';
...
export const deleteOrder = registry.defineAction({
  name: "deleteOrder",
  op: "delete",
  input: z.object({
    "id": z.string(),
  ...
  execute: async ({ input }) => {
    try {
      ...
      return await prisma.order.delete({ where: { "id": input["id"] } });
    } catch (error) {
