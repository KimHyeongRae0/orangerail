/**
 * Orangerail action `updateArticle` (Prisma update on model `Article`).
 *
 * This file is yours — re-scans never modify it; `orangerail sync` reports drift.
 * Write operation: staged for human approval; on approval it runs `prisma.article.update(...)` against your database.
 */
import { z } from 'zod';
...
export const updateArticle = registry.defineAction({
  name: "updateArticle",
  op: "update",
  input: z.object({
    "id": z.number().int(),
