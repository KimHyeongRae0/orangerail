...
- `ontology/*.mjs` — generated by `orangerail init --gate all`; **these files are yours**
  (edit the policies, add `where` guards). All six writes carry `policy:
  { approval: 'required' }` here, which is what makes `6 approval-gated, 0 auto` true
  below — that is this example's deliberate choice, not what `init` gives you by default.
  The default is `--gate delete`, which would gate `deleteArticle` and `deleteComment` and
  leave the four creates and updates executable; `deleteArticle` carries the gate under
  either, and it is the gate that stages it.
...
`orangerail-studio`, all at `0.1.0`), and `npx orangerail init --gate all` sets the same
governance up on your own project without cloning anything — the
[Quickstart](../../README.md#quickstart) is that path end to end, and runs the default
`--gate delete` instead.
